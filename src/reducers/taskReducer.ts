...
export default (state = taskInitialState, action: AppActions): ITask[] => {
	switch(action.type) {
		...
		case constants.UPDATE_TASK:
			return state.map(task => { return task.id == action.task.id ? { ...task, ...action.task } : task });
		case constants.MOVE_TASK:
			return state.map(task => { return task.id == action.taskId ? { ...task, listId: action.listId } : task });
